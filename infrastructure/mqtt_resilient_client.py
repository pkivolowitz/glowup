...
"wants a resilient MQTT subscription."  Non-adapter subscribers have
the same subscriber lifecycle needs — silent TCP half-open kills any
subscriber that rolls its own connect/loop_start pattern with no
on_disconnect and no watchdog.  See the inline comments on
...
class MqttResilientClient:
    ...
    def _create_and_start_client(self) -> None:
        ...
        the pattern that worked reliably in earlier subscribers;
        ``connect_async`` was tried but on
    ...
    def _on_disconnect(
        ...
        the event is visible in journalctl; an earlier subscriber
    ...
    def _recover_from_silence(self) -> None:
        ...
        against mosquitto it produced a zombie session: SUBSCRIBE
        was ACKed, retained messages were delivered, but no live
        publishes ever came through.  The bug recurred 14 hours
        after that fix (2026-04-07 09:14:50) and burned 26 hours of
        sensor data.  Generating a brand-new ``client_id`` on every
        recovery avoids the broker-side session-takeover race that
        caused the zombie.
