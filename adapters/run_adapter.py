...
Adapter-specific dependencies are lazy-imported inside factories so a
missing optional dependency in one adapter doesn't prevent the others
from starting.
...
class AdapterProcess(ProcessAdapterBase):
    ...
    Args:
        adapter_id: Unique adapter identifier (e.g., ``"matter"``).
