...
- ``MqttAdapterBase`` — MQTT subscriber adapters (BLE)
- ``PollingAdapterBase`` — synchronous polling thread (Printer)
- ``AsyncPollingAdapterBase`` — asyncio event loop + reconnect (Matter)
...
class MqttAdapterBase(AdapterBase):
    ...
    is a standalone extraction of this same lifecycle, created for
    subscribers that need the same resilience but could not inherit
    from this class (they are not adapters).  Migrating this
    ...
    and burned 26 hours of dead sensor data.  Tearing down the
    ...
    def _recover_from_silence(self) -> None:
        ...
        against real mosquitto it produced a zombie session: the
        SUBSCRIBE was ACKed, retained messages were delivered, but
        no live publishes ever came through.  The bug recurred 14
        hours after that fix landed (2026-04-07 09:14:50) and burned
        26 hours of sensor data before being noticed.
