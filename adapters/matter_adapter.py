...
class MatterAdapter:
    ...
    def power_on(self, device_name: str) -> bool:
        ...
        Returns:
            True if command sent successfully, False if matter deps
            were missing at import time (start() refused to start
            the background thread; see _HAS_MATTER).
        """
        if not _HAS_MATTER:
            logger.warning(
                "power_on(%r) ignored — matter deps missing; "
                "see startup log for the import that failed",
                device_name,
            )
            return False
    ...
    def power_off(self, device_name: str) -> bool:
        ...
        if not _HAS_MATTER:
            logger.warning(
                "power_off(%r) ignored — matter deps missing; "
                "see startup log for the import that failed",
                device_name,
            )
            return False
    ...
    def toggle(self, device_name: str) -> bool:
        ...
        if not _HAS_MATTER:
            logger.warning(
                "toggle(%r) ignored — matter deps missing; "
                "see startup log for the import that failed",
                device_name,
            )
            return False
