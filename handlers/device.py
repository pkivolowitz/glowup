...
class DeviceHandlerMixin:
    ...
    def _handle_post_power(self, ip: str) -> None:
        ...
        try:
            ...
            # Group power fan-out.  Groups can mix LIFX bulbs and
            # Matter devices; each transport is dispatched to its own
            # subsystem.  The LIFX path goes through the
            # device_manager (virtual emitter or fallback worker);
            # the matter dispatch happens here regardless of whether
            # the LIFX path has a live virtual emitter.
