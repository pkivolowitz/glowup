...
``matter:foyer_motion:occupancy:gated`` only fires when the occupancy
gate passes motion through — no special-case code needed.
...
Config example::

    {
        ...
        "sensor": {"type": "matter", "label": "hallway_motion",
...
def _signal_name_from_sensor(sensor: dict[str, Any]) -> str:
    ...
    The transport (matter, etc.) is metadata on the signal,
