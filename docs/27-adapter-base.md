...
GlowUp integrates with Matter / Thread devices and network
printers.  Each integration is an **adapter** — a small
class that bridges an external data source to the platform's
`SignalBus`.  Before the adapter base classes existed, every adapter
duplicated the same init/start/stop lifecycle, thread management,
...
| `MatterAdapter`  | `AsyncPollingAdapterBase` | Asyncio polling of the Matter / Thread fabric |
...
def _on_started(self):
    logger.info(
        "Printer adapter started -- host %s, polling every %ds",
        self._host, self._poll_interval,
    )
```
