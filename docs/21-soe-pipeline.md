...
(say, a motion detector) instantly composes with every existing
...
are one emitter among many.  The real product is the composable
pipeline that drops into eldercare, acoustic detection, industrial
monitoring, entertainment — any domain where sensors feed compute
that drives action.
...
|---------|--------|-----------|--------|
| MatterAdapter | Matter / Thread devices | AsyncPollingAdapterBase | Deployed |
