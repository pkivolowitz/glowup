...
worker nodes, and external clients can exchange
...
|---------------|--------|------|
| `glowup/signals/{device}:{property}` | Out-of-process adapters | Normalized device signals — published to the hub mosquitto and consumed by the hub's `_on_remote_signal` callback |
...
- media coordination where low-latency UDP is not required
