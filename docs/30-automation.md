...
motion triggers lights on, a watchdog timeout turns them off.  A
general-purpose rule engine drives this, with full CRUD via REST API,
on top of the supported sensor transports (Matter).
...
|-----|------|-------------|
| `type` | `str` | Transport: `"matter"` |
| `label` | `str` | Device label (e.g., `"hallway_motion"`, `"foyer_contact"`) |
