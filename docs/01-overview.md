...
Sensors bring information into the system.  A motion detector, soil
sensor, lock state, microphone, camera feed, REST call, or user-set
parameter are all signals.  Operators transform those signals:
thresholding, gating, occupancy inference, FFT analysis, ML
classification, scheduling, and effect rendering.  Emitters send
results back into the world as light, audio, screens, notifications,
logical signals, or other downstream inputs.
...
- **Generalized signal system** — a temperature reading, a sensor
  event, and a UI parameter change are all just named signals.
- **Decentralized deployment** — sensors, operators, and emitters may run
  on different computers and communicate over MQTT, UDP, HTTP,
  Matter, vendor APIs, and other transports.
