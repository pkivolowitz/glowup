...
and a temperature reading from a Matter sensor are the same type of
...
Examples:
- `hallway_motion:occupancy` — Matter motion sensor
- `hallway_contact:contact` — Matter door sensor
- `front_door:lock_state` — Matter / Thread lock
...
reveal whether the data arrived over Matter, some other radio, or was
typed into a REST endpoint.  The transport is stored in
...
SignalMeta(
    ...
    description="Matter motion",   # human-readable
    source_name="hallway_motion",  # device/source identifier
    transport="matter",            # radio/protocol family
...
- **Transport routing:** `signals_by_transport("matter")` returns all Matter signals.
...
class SensorAdapter(ABC):
    TRANSPORT: str = ""               # subclass sets: "matter", etc.
...
|---------|-----------|-----------|---------------|
| `MatterAdapter` | Matter / Thread | Asyncio polling of the Matter fabric | lock: bool→0.0/1.0, on/off: bool→0.0/1.0 |
...
```json
{
    "printer": {"mqtt_topic_prefix": "glowup/printer"}
}
```
...
```
          ┌──────────────────────────┐
Sensors   │ MatterAdapter            │──► hallway_contact:contact
          │                          │──► hallway_motion:occupancy
          │                          │──► front_door:lock_state
          └──────────┬───────────────┘
                 │ SignalBus
          ┌──────▼───────┐
Operators │ Occupancy    │──► house:occupancy:state
          │ MotionGate   │──► hallway_motion:occupancy:gated
...
- Replace a Matter lock with a lock on another radio → same
  `*:lock_state` signal, OccupancyOperator works unchanged.
- Swap one motion sensor for another vendor → same
  `*:occupancy` signal, MotionGateOperator works unchanged.
...
optional dependency disables that operator without affecting others
or the Operator ABC itself.
...
- **Transport is metadata.** The bus doesn't care how data arrived.  An
  ...
  speaks Matter or some future radio.
