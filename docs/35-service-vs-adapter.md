...
>    subscribe, webhook listener, websocket subscribe).
>
...
(e.g. a host with a radio attached).  It opens a `paho` client to
...
- Hosting a small HTTP API for direct queries that bypass the hub
  (a sensor service can expose `/devices` and `/history` endpoints
  that a client talks to directly rather than asking the hub to
  proxy).
- Carrying its own per-device persistent state (a sensor service
  can own its own history database so the hub does not need to
  replicate it).
...
```
   ...
   │  hub filesystem,    │               │  a sensor host,     │
   │  hub scheduler)     │               │  hardware paired    │
   └──────────┬──────────┘               │  with that host)    │
              │                          │                     │
              │                          └──────────┬──────────┘
              │                                     │
...
A printer that doesn't answer an SNMP poll throws a clear error;
an MQTT subscribe that stops receiving messages does nothing.  The
...
|---------------------------------|------------|--------------------------|----------|-------------------------------------------------------------------------------------------|
| BLE sensor service              | sensor host| BLE radio on that host   | SERVICE  | The BLE radio is paired to a remote host; the host pushes signals to the hub             |
| `MatterAdapter`                 | hub        | Matter devices on LAN    | ADAPTER  | Matter is hub-resident; the hub IS the Matter controller                                  |
...
5. Tests: services typically have a unit-test file at
   ...
   with a stub broker.  Extend the hub-side `_on_remote_signal`
   coverage with an assertion for any new signal shape your
   service introduces.
6. Decide whether the service needs its own staleness probe in
...
- **Be discoverable from `/api/home/health`** in some form — either
  as an adapter entry or a `broker2_signals_last_ts`-style staleness
  probe.  An invisible producer is a silently-failing producer.
