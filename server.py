"""GlowUp REST API server — API host for the local SOE runtime.
...
and distributed workers.  This server subsumes the role of the
...
Architecture::
    ...
    SignalBus / Controller / Operators / Adapters
        ↓ mixed transports
    LIFX / MQTT / Workers / Other emitters
...
# replaced by the trigger operator framework), but the validation
# helper is still load-bearing for the /api/automations CRUD path.
...
from handlers import (
    DeviceHandlerMixin, GroupHandlerMixin,
    SensorHandlerMixin,
...
class GlowUpRequestHandler(
    ...
    # signals.  time:* signals are excluded because they originate
    # from the hub's own scheduler and would mask a silent broker-2
    # outage.  None means "never seen since server start."
...
def main() -> None:
    ...
    def _background_startup() -> None:
        ...
        nonlocal printer_proxy, matter_proxy
        ...
        try:
            ...
            if proc_mqtt is not None:
                # Create proxies for each adapter type.  Out-of-process
                # adapters publish cross-host direct to the hub on
                # glowup/signals/*; the hub consumes them through the
                ...
                def _on_remote_signal(
                    ...
                    # from a broker-2 producer.  time:* signals come
                    # from the hub's own scheduler and would mask a
                    # silent broker-2 outage, so they are excluded.
                    # Stamp before JSON parsing so malformed payloads
                    # still count as "broker-2 is alive and
                    # publishing."  The probe lives on the consumer
                    # side of signals/# so any producer is covered.
