...
helper is the consolidated lifecycle that non-adapter subscribers
use to avoid the silent-death bug (half-open TCP, no on_disconnect
callback, no watchdog, paho's internal autoreconnect never fired).
...
class TestClientConstruction(unittest.TestCase):
    ...
    def test_client_id_has_prefix_epoch_counter(
        ...
        mock_mqtt.Client.return_value = MagicMock()
        with patch("infrastructure.mqtt_resilient_client._PAHO_V2", False):
            client, _ = _make_client(client_id_prefix="subscriber")
            client.start()
        client_id: str = mock_mqtt.Client.call_args[1]["client_id"]
        parts = client_id.split("-")
        self.assertEqual(parts[0], "subscriber")
