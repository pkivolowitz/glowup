...
# ---------------------------------------------------------------------------
# 4. Missing-deps guard — commands return False, never NameError
# ---------------------------------------------------------------------------

class MissingDepsGuardTests(unittest.TestCase):
    """When matter deps fail to import (_HAS_MATTER=False), public command
    methods must refuse cleanly instead of crashing with NameError on the
    unbound ``clusters`` symbol.  Named precedent: 2026-05-05, hub matter
    adapter ran for 3 days with python-matter-server / home-assistant-chip-
    clusters missing from the venv; every dashboard click crashed with
    "name 'clusters' is not defined".
    """

    def _adapter_without_deps(self) -> "MatterAdapter":  # type: ignore[name-defined]
        """Construct an adapter with _HAS_MATTER forced False.

        Mirrors the runtime situation when an import fails: the module-
        level symbols (clusters, MatterClient, aiohttp) are unbound and
        any code path that names them raises NameError.
        """
        _install_matter_stubs()
        from adapters.matter_adapter import MatterAdapter
        import adapters.matter_adapter as m
        # Force the runtime state we're guarding against.
        m._HAS_MATTER = False
        # Strip the stubbed clusters back out so an unguarded reference
        # would NameError just like in production.
        if hasattr(m, "clusters"):
            del m.clusters
        return MatterAdapter(_CONFIG, bus=None)

    def test_power_on_returns_false_without_deps(self) -> None:
        """power_on must not NameError when _HAS_MATTER is False."""
        adapter = self._adapter_without_deps()
        try:
            result: bool = adapter.power_on("Backyard Lights 1")
        finally:
            # Restore stubs so later tests don't see a half-stripped module.
            _install_matter_stubs()
        self.assertFalse(result)

    def test_power_off_returns_false_without_deps(self) -> None:
        adapter = self._adapter_without_deps()
        try:
            result: bool = adapter.power_off("Backyard Lights 1")
        finally:
            _install_matter_stubs()
        self.assertFalse(result)

    def test_toggle_returns_false_without_deps(self) -> None:
        adapter = self._adapter_without_deps()
        try:
            result: bool = adapter.toggle("Backyard Lights 1")
        finally:
            _install_matter_stubs()
        self.assertFalse(result)
