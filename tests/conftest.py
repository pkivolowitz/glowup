...
* Point ``GLOWUP_SITE_JSON`` at ``tests/_test_site.json`` so the
  ...
  ``/etc/glowup/site.json``: several modules require ``latitude``
  and ``longitude`` at module import — without this fixture every
  test that touches them raises ``SiteConfigError`` at collection.
